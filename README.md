...
> // example // I like adding these //
 ...
 Tests: Methods with return values are evaluated for if the return exactly matches the expected value, and void methods are tested for if the class values are updated correctly. 
...
Tests: Methods with return values are evaluated for if the return exactly matches the expected value, and void methods are tested for if the class values are updated correctly. All variations of TokenTypes with unique updating methods are tested as well.
...
Tests: Methods with return values are evaluated for if the return exactly matches the expected value, and void methods are tested for if the class values are updated correctly. Differing tokens are tested to find unique counts and non-unique counts.
